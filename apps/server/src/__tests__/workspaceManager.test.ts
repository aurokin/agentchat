...
`,
    ...
    test("refreshes the sandbox root heartbeat for idle instances", async () => {
        ...
        const readUpdatedAt = () => {
            ...
            return registry.activeInstances?.["instance-a"]?.updatedAt ?? 0;
        };

        const initialUpdatedAt = readUpdatedAt();
        await new Promise((resolve) => setTimeout(resolve, 5));

        manager.refreshSandboxRootsHeartbeat();

        expect(readUpdatedAt()).toBeGreaterThan(initialUpdatedAt);
    });
    ...
    describe("deleteWorkspace", () => {
        ...
        test("is a no-op for shared-mode agents with unsafe ids", async () => {
            const sandboxRoot = makeTempDir("sandbox");
            const manager = createWorkspaceManager(() =>
                makeConfig({
                    sandboxRoot,
