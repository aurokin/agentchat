...
// The pre-portless harness (removed in commit 6760428) wrote env files with
// this header. Anything inside is pre-portless and must not be preserved —
// stale URLs/secrets there cause confusing 404/401 errors at runtime.
const LEGACY_BOOTSTRAP_HEADER =
    "# This file is generated by bun run bootstrap.";

function readDotenvIgnoringLegacyBootstrap(absPath: string): Dotenv {
    if (!fs.existsSync(absPath)) return {};
    const content = fs.readFileSync(absPath, "utf8");
    if (content.startsWith(LEGACY_BOOTSTRAP_HEADER)) return {};
...
async function main(): Promise<void> {
    ...
    const existingServerEnv = readDotenvIgnoringLegacyBootstrap(serverEnvPath);
    const preserveOrFallback = (
